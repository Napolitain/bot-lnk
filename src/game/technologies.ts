...
import { Technology } from '../generated/proto/config.js';
...
export async function getResearchedTechnologies(
  ...
  try {
    ...
    const techRows = page.locator('.menu-list-element-basic.clickable.with-icon-left');
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (techEnum === undefined) {
        console.log(`[getResearchedTechnologies] Unknown technology: ${techName}`);
        continue;
      }
      ...
      if (!hasButton) {
        ...
        console.log(`[getResearchedTechnologies] Already researched: ${techName}`);
      }
    }

    console.log(`[getResearchedTechnologies] Found ${researched.length} researched technologies`);
  } catch (e) {
