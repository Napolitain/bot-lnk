...
export function printUnitsRecommendation(castleName: string, rec: UnitsRecommendation): void {
...
export function printUnitComparison(
  ...
  for (const [unitType, data] of comparison) {
    ...
    console.log(`  ${unitTypeToJSON(unitType)}: ${data.current}/${data.recommended} ${status}`);
  }
}

/** Print cycle summary */
export function printCycleSummary(upgrades: number, recruits: number, trades: number): void {
  console.log(`\n=== Cycle Summary ===`);
...
/** Print sleep info */
export function printSleepInfo(sleepMs: number, timeRemainingMs: number, freeFinishAvailable: boolean): void {
